...
left_content = (
    ...
    "| Hr | Cond | Temp | Short<br>wave | Est | Prod | Con<br>sump | Act<br>Cons | Forecast | Actual | +/- |\n"
    "|--:|:--|--:|--:|--:|--:|--:|--:|--:|--:|--:|\n"
    "{% for h in d.hours %}"
    "| {{ h.hour }} | {{ h.condition[:5] }} | "
    "{{ h.temp if h.temp else '-' }} | "
    "{{ h.sw_wm2 if h.sw_wm2 else '-' }} | "
    "{{ h.solar_active if h.solar_active is defined else h.solar_rad }} | "
    ...
    "{% else %}"
    "uses shortwave radiation intensity to estimate "
    "**{{ d.active_total if d.active_total is defined else d.rad_total }} kWh** of usable peak solar"
    "{% endif %}. "
    ...
    "{% endif %}\n\n"
    "{% if d.live_projected_solar is defined %}"
    "Live solar today is **{{ d.actual_solar_so_far }} kWh so far**. "
    "Projecting actual production so far plus forecast remaining gives "
    "**{{ d.live_projected_solar }} kWh**, "
    "{% if d.live_projected_solar > (d.active_total if d.active_total is defined else d.rad_total) + 2 %}"
    "so today is running materially ahead of the frozen model estimate."
    "{% elif d.live_projected_solar < (d.active_total if d.active_total is defined else d.rad_total) - 2 %}"
    "so today is running materially behind the frozen model estimate."
    "{% else %}"
    "roughly in line with the frozen model estimate."
    "{% endif %}\n\n"
    "{% endif %}"
    ...
    "{% set reserve_target = d.reserve_target_pct if d.reserve_target_pct is defined else 45 %}"
    "{% set battery_reserve = d.battery_reserve_pct if d.battery_reserve_pct is defined else 10 %}"
    "{% if d.sim_min_soc <= battery_reserve %}"
    "Even from **{{ plan.overnight_soc }}%**, the simulation reaches the inverter reserve "
    "(minimum **{{ d.sim_min_soc }}%** at {{ d.sim_min_hour }}:00"
    "{% if d.sim_shortfall_kwh is defined and d.sim_shortfall_kwh is not none and d.sim_shortfall_kwh > 0 %}"
    ", with up to **{{ d.sim_shortfall_kwh }} kWh** of unmet battery demand below reserve"
    "{% endif %}). "
    "This is a capacity-limited day: peak grid import is expected unless actual solar is better "
    "or actual load is lower than forecast."
    "{% elif d.sim_min_soc < reserve_target %}"
    "The battery stays above the inverter reserve, but only reaches **{{ d.sim_min_soc }}%** "
    "at {{ d.sim_min_hour }}:00, below the **{{ reserve_target }}%** planning buffer "
    "(10% reserve + {{ d.safety_margin_pct if d.safety_margin_pct is defined else 35 }}% safety margin). "
    "There is limited forecast-error margin."
    "{% else %}"
    "The battery remains comfortable, with a minimum of **{{ d.sim_min_soc }}%** at {{ d.sim_min_hour }}:00, "
    "above the **{{ reserve_target }}%** planning buffer."
    "{% endif %}"
    "{% if plan.overnight_soc >= 100 and d.sim_min_soc < reserve_target %}"
    " The target is therefore capped at **100%**: the optimiser cannot choose a higher overnight SOC, "
    "so any remaining risk must be handled by actual solar, load shifting, or unavoidable peak import."
    "{% else %}"
    " The target of {{ plan.overnight_soc }}% was chosen by binary search as the lowest SOC "
    "that keeps the model above the **{{ reserve_target }}%** planning buffer, avoiding unnecessary "
    "grid charging when free solar is forecast."
    "{% endif %}"
...
middle_content = (
    ...
    "| **Model estimate** | **{{ d.active_total if d.active_total is defined else d.rad_total }} kWh** |\n"
    "{% if d.live_projected_solar is defined %}| **Live projection** | **{{ d.live_projected_solar }} kWh** |\n{% endif %}"
    "| Cloud model | {{ d.cloud_total }} kWh |\n"
    "| Radiation model | {{ d.rad_total if d.rad_total else 'N/A' }} kWh |\n"
    "| **Active model** | **{{ learn.active_model }}** |\n"
    ...
    "| Planning buffer | {{ d.reserve_target_pct if d.reserve_target_pct is defined else 45 }}% |\n"
    "| **Sim min SOC** | **{{ d.sim_min_soc }}%** at {{ d.sim_min_hour }}:00 |\n"
    "| Sim shortfall | {{ d.sim_shortfall_kwh if d.sim_shortfall_kwh is defined and d.sim_shortfall_kwh is not none else '-' }} kWh |\n"
...
right_content = (
    ...
    "| SW scale | {{ learn['sw_scale_' + season] }} |\n\n"
    "### Other\n\n"
    "| | |\n"
    "|:--|--:|\n"
