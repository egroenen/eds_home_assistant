...
left_content = (
    ...
    "| Hr | Cond | Cloud | Temp | Short<br>wave | Cloud | Rad | Con<br>sump | Forecast | Actual | +/- |\n"
    "|--:|:--|--:|--:|--:|--:|--:|--:|--:|--:|--:|\n"
    "{% for h in d.hours %}"
    "| {{ h.hour }} | {{ h.condition[:5] }} | {{ h.cloud }}% | "
    ...
    "{% endif %}\n\n"
    "Daily consumption averages **{{ learn.consumption_avg }} kWh**, adjusted by: "
    "seasonal factor {{ d.seasonal }} "
    "({{ 'late summer' if d.seasonal < 1.0 else 'winter' if d.seasonal > 1.05 else 'shoulder season' }})"
    "{% if d.avg_temp %}"
    ", temperature factor {{ d.temp_factor }} "
    "(avg {{ d.avg_temp }}C — "
    ...
    "{% endif %}"
    ". This gives **{{ d.consumption_total }} kWh** of peak consumption.\n\n"
