...
detail_content = (
    ...
    "**Peak consumption (7am–9pm):** {{ d.consumption_total }} kWh "
    "({{ d.season }} base {{ d.seasonal_avg }} kWh/day, temp {{ d.temp_factor }}"
    "{% if d.avg_temp %}, avg {{ d.avg_temp }}°C{% endif %})\n\n"
    "**Simulation:** min SOC {{ d.sim_min_soc }}% at {{ d.sim_min_hour }}:00, "
    "target {{ plan.overnight_soc }}%\n\n"
    "### Hourly Forecast\n\n"
    "| Hour | Weather | Temp | SW W/m² | ☀️ Rad | ⚡ Prod | Cons | Act Cons | Battery |\n"
    "|:--:|:--|--:|--:|--:|--:|--:|--:|--:|\n"
    "{% for h in d.hours %}"
    "| {{ '%02d' | format(h.hour) }} | {{ h.condition[:6] }} |"
    " {{ '%.0f' | format(h.temp) if h.temp else '-' }}° |"
    " {{ '%.0f' | format(h.sw_wm2) if h.sw_wm2 else '-' }} |"
    " {{ '%.2f' | format(h.solar_rad) }} |"
    " {{ '%.2f' | format(h.actual_pv_kwh) if h.actual_pv_kwh is not none else '-' }} |"
    ...
    "| Consumption ({{ state_attr('sensor.solar_optimizer_status', 'detail').season | default('autumn') }}) | {{ learn['consumption_' + (state_attr('sensor.solar_optimizer_status', 'detail').season | default('autumn'))] }} kWh |\n"
...
optimizer_card = {
    ...
    "content": (
        ...
        "| Raw solar | {{ '%.1f' | format(det.raw_solar if det else plan.solar_forecast) }} kWh | |\n"
